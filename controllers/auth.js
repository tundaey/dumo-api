...
let registerUser = (email, password, account_type) => {
	return new Promise((res, rej) => {
		...
		return Users.find({ email: email })
			.then(user => {
				...
				let newUser = { email, profileComplete: false, account_type };
...
let createToken = user => {
	return jwt.sign(_.omit(user.toObject(), 'password'), config.secret, {
		expiresIn: '20h' //lower value for testing
	});
};
